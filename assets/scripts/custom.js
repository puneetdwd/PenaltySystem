...
function int_n_float_only(event){
 ...
 function map_part_foolproof(part_id){
  ...
  function on_handle_checkbox(e,audit_id)
    {
        var base_url = $('#base_url').val();
        var s = $('#send_gerp').parent('span').attr('class');
        var m = base_url+'auditer/mark_as_complete';
        if(s != 'checked'){
            //alert(123);
            bootbox.confirm({ 
                      title: "Confirm Box",
                      message: "Are you sure you want to sent it to GERP. Once sent can't be undo.", 
                      callback: function(){ /* your callback code */ }
                    });
            
            mm = m+'/'+audit_id;
            $('#mark_comp').prop("href", mm);
            $('#mark_comp1').prop("href", mm);
        }
        else{
            //alert(456);
            $('#mark_comp').prop("href", m);
            $('#mark_comp1').prop("href", m);       
        }
    }
